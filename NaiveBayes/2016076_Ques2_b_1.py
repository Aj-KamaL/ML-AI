...
# Randomized K-fold
cpp = list(zip(fa_train, ia_train))
random.shuffle(cpp)
fc_train, ic_train = zip(*cpp)
...
for i in range (0,5):    
    ...
    il_train=[]
    il_test=[]
    print( "Fold : ", str(2518*i) , str(2518*(i+1)))
    for j in range (0,len(fc_train)):
        
        if 2518*i >j or j>=2518*(i+1):
            ff_train.append(fc_train[j])
            il_train.append(ic_train[j])
            
        else:
            il_test.append(ic_train[j])
            ff_test.append(fc_train[j])
            
#     print(len(ff_train),len(ff_test))
...
global_indx=Accuracy.index(max(Accuracy))
print("Validation")
...
ff_train=f_ff_train[global_indx]
il_train=f_il_train[global_indx]
...
threshold=[0.41,0.42,0.43,0.45,0.475,0.48,0.49,0.5,0.51,0.52,0.53,0.54,0.55,0.56,0.6,0.7]
